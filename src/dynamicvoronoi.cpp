...
void DynamicVoronoi::initializeMap(int _sizeX, int _sizeY, float _resolution, bool** _gridMap) {
  gridMap = _gridMap;
  ...
  // cv::Mat imgVoronoi(cv::Size(sizeX, sizeY), CV_32FC1, cv::Scalar(0.0f));
  // for(int row = 0; row < sizeY; ++row)
  // {
  //   for(int col = 0; col < sizeX; ++col)
  //   {
  //     float dist = data[col][row].dist;
  //     if(!isinff(dist))
  //         imgVoronoi.ptr<float>(sizeY - row - 1)[col] = data[col][row].dist;
  //     else
  //         imgVoronoi.ptr<float>(sizeY - row - 1)[col] = 1.0;
  //   }
  // }
  // {
  //   int debug = 0;
...
// 这个函数是最关键的，怎么把那个edge找出来呀？？
// 那动态的意思是可以更新，怎么传入数据呢？以一个什么频率呢？更新产生的是什么呢？
...
void DynamicVoronoi::visualize(const char *filename) {
  // write pgm files
  filename = "/home/bhrqhb/catkin_ws/src/Hybrid_A_Star/maps/result.ppm";
  ...
  fprintf(F, "P6\n");
  fprintf(F, "%d %d 255\n", sizeX, sizeY);
  cv::Mat imgVoronoi(cv::Size(sizeX, sizeY), CV_8UC3, cv::Scalar(0, 0, 0));
  for(int y = sizeY-1; y >=0; y--){      
    for(int x = 0; x<sizeX; x++){	
      ...
      if (isVoronoi(x,y)) {
        ...
        fputc( 0, F );
        fputc( 0, F );
        imgVoronoi.at<cv::Vec3b>(sizeY - y - 1, x) = cv::Vec3b(0, 0, 255);
      }
      //障碍物的地方 
        ...
        fputc( 0, F );
        fputc( 0, F );
        imgVoronoi.at<cv::Vec3b>(sizeY - y - 1, x) = cv::Vec3b(0, 0, 0);
      } 
      //其他区域，按照距离值来赋予色彩
      else {
        ...
        fputc( c, F );
        fputc( c, F );
        imgVoronoi.at<cv::Vec3b>(sizeY - y - 1, x) = cv::Vec3b(c, c, c);
      }
    }
  }
  fclose(F);
